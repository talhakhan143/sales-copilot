...
/**
 * Server side proxy for POST /api/practice/start.
 *
 ...
 * message the setup form can render as a calm notice.
 *
 * The timeout is 40 seconds because this route does everything prepare context
 * does, the client web page is fetched and the system prompt is fused, and only
 * then is the practice persona built on top.
 *
...
export async function POST(request: Request): Promise<Response> {
  ...
  try {
    upstream = await fetch(`${BACKEND_HTTP_URL}/api/practice/start`, {
      method: "POST",
